...
def compute_posterior(prior, likelihood, y):
    ...
    for true_rating in range(M):
        terms[true_rating] = log_prior[true_rating]

        for user_rating in y:
            terms[true_rating] += log_likelihood[user_rating, true_rating]

    log_answer = terms - scipy.misc.logsumexp(terms)
    #
...
def compute_movie_rating_likelihood(M):
    ...
    alphas = 1 / likelihood.sum(axis = 0)
...
def main():
    ...
    for i in range(5):
        print("***", i, "***")
        entropies[i] = np.mean(compute_true_movie_rating_posterior_entropies(observations[i]))
