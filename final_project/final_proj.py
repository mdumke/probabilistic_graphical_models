...
def compute_empirical_distribution(values):
    ...
    for key, count in distribution.items():
        distribution[key] = count / num_values
...
def compute_empirical_mutual_info_nats(var1_values, var2_values):
    ...
    p_var1_var2 = compute_empirical_distribution(zip(var1_values, var2_values))
    ...
    # compute entropies
    joint_entropy = 0.0
    independent_entropy = 0.0

    for x in p_var1.keys():
        for y in p_var2.keys():
            p = p_var1_var2.get((x, y))
            q = p_var1.get(x) * p_var2.get(y)
...
def rank_edges_by_empirical_mutual_information (observations):
    """
    Computes mutual information for all pairs of nodes

    Input
    -----
    Ovservations: a 2D NumPy array (see chow-liu)

    Output
    ------
    Returns an array of node-tuples as [(n1, n2), (n1, n3), ...)] sorted
    by the amount of empirical mutual information
    """
    edges = []

    # compute mutual information for all possible node-pairs
    for i in range(observations.shape[1] - 1):
        for j in range(i + 1, observations.shape[1]):
...
def chow_liu(observations):
    ...
    ranked_edges = rank_edges_by_empirical_mutual_information(observations)

    # add edges if they don't already exist in the set
    for i, j in ranked_edges:
